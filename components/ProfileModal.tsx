...
import { XIcon, LockIcon, HeartIcon, MessageCircleIcon, CheckBadgeIcon, MoreVerticalIcon, ShareIcon, InfoIcon } from '../constants';
...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose, user, onBlockUser, onSendMessage, onOpenAbout }) => {
    const [isOptionsOpen, setIsOptionsOpen] = useState(false);
    
    if (!isOpen || !user) return null;

    const userPosts = MOCK_POSTS.filter(p => p.user.id === user.id);
    const showStats = !user.isPrivate;
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    ...
                    url: window.location.href, // Or a direct profile URL
                });
            } catch (error) {
                        ...
                        <div className="relative">
                            ...
                            {isOptionsOpen && (
                                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-zinc-800 rounded-lg shadow-lg border dark:border-zinc-700 z-20">
                                    <button onClick={() => { onOpenAbout(user); setIsOptionsOpen(false); }} className="w-full flex items-center gap-3 text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-zinc-700 rounded-t-lg"><InfoIcon className="w-5 h-5"/> About this Profile</button>
                                    <button onClick={handleShare} className="w-full flex items-center gap-3 text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-zinc-700"><ShareIcon className="w-5 h-5"/> Share Profile</button>
